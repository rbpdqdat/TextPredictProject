...
getProbabilityFrom3Gram("and a case of")
Rprof(tmp <- tempfile())
getProbabilityFrom3Gram("you must be")
Rprof()
summaryRprof(tmp)
...
#  mylist = separateTerms(getLastTerms(inputString, num = 2))
  ...
  #produces a new table with all of the matches on the firstTerms 
  #     (if there are any matches)
  oneGroupIn3Gram = threeGramTable[firstTerms == inFirstTerms3gram]
  
  # if there are matches to the 'firstTerms' then 'oneRecordIn3Gram' is produced 
  #   from the threeGramTable that has both matches on the 'firstTerms' and
  #   the 'lastTerm'
  if (nrow(oneGroupIn3Gram) > 0){
    # Algorithm here
    # If there is a one gram 
    firstTermMatches = threeGramTable[firstTerms == inFirstTerms3gram]
    inLastTerm3gram = firstTermMatches[1]$lastTerm
    oneRecordIn3Gram = threeGramTable[firstTerms == inFirstTerms3gram & lastTerm == inLastTerm3gram]
    if (nrow(oneRecordIn3Gram) > 0){
      # We found at least one in 3-gram
    ...
    oneGroupIn2Gram = twoGramTable[firstTerms == inFirstTerms3gram]
    
    if (nrow(oneGroupIn2Gram) > 0){
      # Algorithm here
      # If there is a one gram 
      oneRecordIn3Gram = twoGramTable[firstTerms == inFirstTerms3gram & lastTerm == inLastTerm3gram]
      if (nrow(oneRecordIn3Gram) > 0){
        # We found at least one in 3-gram
        all_freq = sum(oneGroupIn2Gram$frequency)
        finalProb = ((oneRecordIn3Gram$discount * oneRecordIn3Gram$frequency) / all_freq)
        ...
        # NOT found in 2-gram => check 1-gram 
          # We only have hope in 1-gram!
          oneGroupIn1Gram = oneGramTable # we don't have "firstTerms" here!
          oneRecordIn1Gram = oneGramTable[lastTerm == inLastTerm2gram] # what if this returns "zero" row?
...
#getLastTerms = function(inputString, num = 2){
getLastTerms = function(inputString, num = 2){
  # Preprocessing
