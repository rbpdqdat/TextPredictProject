...
trigramFile <- paste(c(dir0,'trigram.hunspell.Rda'),collapse='')
load(trigramFile)
row.names(wf) <- NULL
#trigram.df <- head(wf)
...
head(trigram.df)
system.time ({
  trigram.df$firstTerms <- lapply(as.character(trigram.df$Token), 
    function(x) {paste(strsplit(x," ")[[1]][1],strsplit(x," ")[[1]][2],sep="_")}
  )
  trigram.df$lastTerm <- lapply(as.character(trigram.df$Token), 
    function(x) {strsplit(x," ")[[1]][3]})
})

names(trigram.df)[names(trigram.df) == 'Frequency'] <- 'frequency'
head(trigram.df)
trigram.df <- as.data.frame(lapply(trigram.df, unlist))
trigram.df <- trigram.df[order(trigram.df$firstTerms),]
threeGramTable <- setDT(trigram.df)
#remove word column
threeGramTable[,Token:=NULL]
head(threeGramTable)
#typeof(head(threeGramTable$firstTerms[[1]]))
#head(threeGramTable$firstTerms)

#createThreeGramTableExtended()
